...
def find_remote_log(cfg: VmConfig) -> tuple[str, float]:
    """For --attach: locate the newest site run log on the VM and recover the
    ...
    cp = ssh_run(cfg, f"ls -t {REMOTE_LOGDIR}/*_site.log 2>/dev/null | head -1", check=False)
    remote_log = cp.stdout.strip()
    if not remote_log:
        raise RuntimeError("No site run log found on the VM -- nothing to attach to.")
...
def finish_success(rs: RunStatus, run_id: str, cfg: VmConfig) -> int:
    ...
    # (alla storlekar, inga dubbletter -- VM-mappen är ren). Din syfte-B-vision (site).
    try:
    ...
    try:
        val_note = run_local_validation("site", local_file)
...
def fetch_all_outputs(cfg: VmConfig, run_id: str) -> tuple[list[Path], str]:
    ...
    remote_dir = "/home/azureuser/bcg/site/output"
    ...
    for f in fetched:
        ...
        prefix = f"{date_folder}/site" + (f"/{subdir}" if subdir not in ("", ".") else "")
...
def _leave_running(rs: RunStatus, cfg: VmConfig, why: str) -> int:
    ...
    rs.mark_waiting(f"{why} Job continues detached on the VM. "
                    f"Re-attach with: py -3.11 orchestration\\runners\\run_site_model.py --attach")
    try:
        write_status(rs)
    except Exception:
        pass
    ...
    print("    py -3.11 orchestration\\runners\\run_site_model.py --attach")
