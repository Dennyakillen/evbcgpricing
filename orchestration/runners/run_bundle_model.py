"""
run_bundle_model.py -- Phase runner: Bundle model steps 1-4 on the Azure VM
========================================================================
Runs the Bundle family's BCG launcher (regular_price -> data_prepration ->
...
log = logging.getLogger("run_bundle_model")
...
REMOTE_CODE    = "/home/azureuser/bcg/bundle/code"
...
EXPECTED_KEYS  = 125          # cluster GROWING steps-1-4 (F.7 ref; 3812 was the FROZEN facit -- wrong baseline)
...
def finish_success(rs: RunStatus, run_id: str, cfg: VmConfig) -> int:
    ...
    try:
        val_note = run_local_validation("bundle", local_file)
...
def fetch_all_outputs(cfg: VmConfig, run_id: str) -> tuple[list[Path], str]:
    ...
    remote_dir = "/home/azureuser/bcg/bundle/output"
    ...
    for f in fetched:
        ...
        prefix = f"{date_folder}/bundle" + (f"/{subdir}" if subdir not in ("", ".") else "")
...
def main() -> int:
    ...
    ap.add_argument("--max-hours", type=float, default=2.0, help="Bundle ref: liten (125 KEY); marginal.")
