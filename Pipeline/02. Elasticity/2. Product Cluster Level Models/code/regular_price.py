...
if __name__ == '__main__':    
    ...
    data_path1 = os.path.join(module_path, config['regular_price']['raw_input_data1'].lstrip("/\\"))
    print(f'Raw Data path: {data_path}')
    print(f'Raw Data path 1: {data_path1}')
    price_level = os.path.join(module_path, config['regular_price']['price_level'].lstrip("/\\"))
    save_path = os.path.join(module_path, config['regular_price']['output_data'].lstrip("/\\"))
    print(f"Data is stored at path: {save_path}")
    
    # Read data
    ads = read_data(data_path)
    ads1 = read_data(data_path1)
    ads = clean_columns(ads)
    ads1 = clean_columns(ads1)
    ads=pd.concat([ads,ads1])
    print(f"Shape of ADS afetr concatenating: {ads.shape}")
