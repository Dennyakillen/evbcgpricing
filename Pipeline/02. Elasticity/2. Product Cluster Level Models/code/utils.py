...
def model(control, df, config, scale=False):
    ...
    for i, x in enumerate(
        ...
        # PoC fix (Jens Palmo): VIF runs on the params-scaled X_temp (utils.py reuses the
        # variable name at the baseline/incremental step), which is numerically singular on
        # our DW data -> matrix_rank raises "zero-size array". VIF is post-hoc diagnostics and
        # does NOT affect the elasticity (already in df_coef above). Guard it: NaN on failure,
        # keep the run alive. Verbatim BCG behaviour preserved where it succeeds.
        try:
            tmp_vif["VIF"] = [variance_inflation_factor(X_temp.values, i)
                              for i in range(len(X_temp.columns))]
        except Exception as _vif_err:
            print(f"VIF skipped for {x}: {type(_vif_err).__name__}")
            tmp_vif["VIF"] = float("nan")       
