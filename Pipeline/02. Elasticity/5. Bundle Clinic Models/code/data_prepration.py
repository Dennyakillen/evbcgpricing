...
if __name__ == '__main__':
    ...
    # Get Seasonality at Key level
    print('shape Before adding dummy rows.', df_with_features.shape)
    
    df_with_features['unit_temp']=np.where(df_with_features['month_end_week']==1,None,df_with_features[UNIT])
    df_with_features = inflate_missing_weeks(df_with_features, START_DATE, END_DATE, DATE)
    print('shape Before adding dummy rows.', df_with_features.shape)
    
    df_with_features = rolling_seasonality(df_with_features, 'unit_temp', KEY, DATE, 3,  3, "fwbw")
    df_with_features = rolling_seasonality(df_with_features, 'unit_temp', KEY, DATE, 6,  3, "fwbw")
    df_with_features = rolling_seasonality(df_with_features, 'unit_temp', KEY, DATE, 12, 3, "fwbw")
    df_with_features = rolling_seasonality(df_with_features, 'unit_temp', KEY, DATE, 18, 3, "fwbw")
    df_with_features = rolling_seasonality(df_with_features, 'unit_temp', KEY, DATE, 24, 3, "fwbw")
    print(df_with_features['INFLATED'].value_counts())
    df_with_features = df_with_features[df_with_features['INFLATED']==0]
    print(df_with_features['INFLATED'].value_counts())
    df_with_features.drop(columns = ['INFLATED', 'FORWARD_ROLLING', 'BACKWARD_ROLLING'], inplace = True)
    print('shape after removing dummy rows.', df_with_features.shape)
