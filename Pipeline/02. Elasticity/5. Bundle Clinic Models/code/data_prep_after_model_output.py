...
def rank_calc(path):
    ...
    # Aggregate by description + code, then rank by revenue
    df = df.groupby([DESC_COL_PRODUCT, Product_Code_var]).agg({DOLLAR: "sum"}).reset_index()
    df[RANK] = df[DOLLAR].rank(ascending=False, method="first")

    # Save rank file for reference
    df.to_csv(rf".\output\rank_{config['itr_name']}.csv", index=False)
    return df
...
def model_output(path, prod_df):
    ...
    df1['Significant ?'] = np.where((df[RSQ_COL].round(2) >= 0.5) & (df[PVALUE_PREFIX + ALGO_REGULAR_PRICE].round(2) <= 0.2), 1,0 )
    df1.rename(columns = {Cluster_Granularity:'Cluster'}, inplace = True)
    return df1
...
def bundle_group_mapping(df, df1, mapping_path=BUNDLE_GROUP_MAPPING_PATH):
    """
    Enrich bundle outputs with group mapping (Bundle only).
    - Reads mapping file from constants
    - Joins with model summary and output
    - Restricts to standardized final columns (from constants.py)
    - Saves both summary and output to Excel
    """

    # Read mapping file
    df_map = pd.read_excel(mapping_path)

    # Merge mapping into both summary + output
    df = pd.merge(df, df_map, on=Product_Code_var, how="left")
    df.rename(columns = {Cluster_Granularity:Cluster_Granularity2}, inplace = True)
    df1 = pd.merge(df1, df_map, on=Product_Code_var, how="left")
    df1 = df1[FINAL_OUTPUT_COLS]

    print("Bundle group mapping applied and files saved.")
    return df, df1
...
if __name__ == "__main__":
    print(f"========== Running pipeline for 1 file collated files ==========")
    # Resolve module_path relative to config file location
    module_path = (config_file.parent / config["module_path"]).resolve()
    ...
    rank_source_path = raw_data_path          # bundle weekly raw input (xlsx)
    # -------------------------------------------------------------
    ...
    ### 2) Reading Raw Data
    raw_data = read_data(raw_data_path, sheet = 'Sheet1')
    ...
    # 5) Bundle group mapping only for Bundle
    model_result_1, model_out = bundle_group_mapping(model_result_1, model_out)
    ...
    template_path = base_dir_output/"Excel_Outputs/Sweden_bundle_level_elasticity_summary_final.xlsb"
