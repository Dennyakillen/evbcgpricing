"""
run_site_model.py -- Phase runner: Site model steps 1-4 on the Azure VM
========================================================================
Runs the Site family's BCG launcher (regular_price -> data_prepration ->
...
Production test design (first run): re-run Site on the data already on the
...
Inputs (on VM, from the 2026-06-09 session):
  ~/bcg/site/data/0902_Sweden_weekly_model_data_site_level.csv  (growing CSV)
  ~/bcg/site/code/control_files/  (control file from prior run)
Outputs:
  VM:    ~/bcg/site/output/model/output_summary.xlsx
  Local: <repo>\\Pipeline\\02. Elasticity\\3. Product Site Level Models\\output\\azure_run_model\\
  Blob:  output/<YYYY-MM-DD>/site_output_summary.xlsx  (dated folder, Jens's spec)
...
Cluster gets its own near-identical runner once this one is proven.

Usage (global Python 3.11, from repo root):
    py -3.11 orchestration\\runners\\run_site_model.py --check     (preflight only)
    py -3.11 orchestration\\runners\\run_site_model.py --dry-run   (show plan)
    py -3.11 orchestration\\runners\\run_site_model.py             (real run, ~70 min)
...
from azure_vm import (                                                    # noqa: E402
    ...
    vm_power_state, wait_for_ssh, ssh_run, ssh_launch_detached, scp_from_vm,
)

log = logging.getLogger("run_site_model")

# ----------------------- Phase constants (Site) ---------------------------
# Cluster's future runner = copy of this file with these constants changed.
PHASE_KEY      = "site_model"
REMOTE_CODE    = "/home/azureuser/bcg/site/code"
REMOTE_PYTHON  = "/home/azureuser/bcg/cluster/.venv/bin/python"  # site shares cluster venv (FAS 13)
REMOTE_INPUT   = "/home/azureuser/bcg/site/data/0902_Sweden_weekly_model_data_site_level.csv"
REMOTE_OUTPUT  = "/home/azureuser/bcg/site/output/model/output_summary.xlsx"
REMOTE_LOGDIR  = "/home/azureuser/bcg/logs"
LOCAL_OUT_DIR  = Path(r"C:\Projekt\BCG\Pipeline\02. Elasticity\3. Product Site Level Models\output\azure_run_model")
EXPECTED_KEYS  = 6624          # 2026-06-09 reference -- reproduction target
...
def get_or_create_status(run_id: str) -> RunStatus:
    ...
    try:
        ...
        return default_pipeline(run_id=run_id, triggered_by="jens (run_site_model)")
...
def launch(cfg: VmConfig, run_id: str, start_date: str, end_date: str) -> str:
    ...
    remote_log = f"{REMOTE_LOGDIR}/{run_id}_site.log"
...
def poll_until_done(cfg: VmConfig, rs: RunStatus, remote_log: str,
                    ...
                    poll_s: int) -> tuple[bool, str]:
    """Poll: process alive? log progress? output fresh? Writes heartbeat +
    sub-progress (launcher's own 'Finished <script>' lines) every tick.
    Returns (success, last_log_tail)."""
    ...
    while _now_utc() < deadline:
        ...
        if fresh and (not running or BENIGN_STEP5 in last_tail):
            # Steps 1-4 done. Step-5 xlwings failure on Linux is EXPECTED
            # (LB.44) -- it is its own LOCAL phase, not this runner's job.
    ...
    return False, last_tail + "\n[runner] TIMEOUT after max-hours"
...
def verify_and_fetch(run_id: str) -> tuple[Path, str]:
    ...
    try:
        ...
        note = f"{keys} unique KEY ({match} 2026-06-09 reference), {note}"
    ...
    return local_file, f"{note}; blob: {', '.join(blob_paths) or 'upload failed'}"


def main() -> int:
    ap = argparse.ArgumentParser(description="Run Site model steps 1-4 on the VM (Phase Z runner).")
    ...
    if args.dry_run:
        ...
        print(f"  poll {args.poll_seconds}s, max {args.max_hours}h; benign: step-5 xlwings; "
              ...
              f"deallocate={'no (--keep-vm)' if args.keep_vm else 'yes'}")
        return 0
    ...
    vm_started_here = vm_power_state(cfg) != "VM running"
    success = False
    try:
        start_vm(cfg)
        wait_for_ssh(cfg)
        ...
        for attempt in (1, 2):                     # two-pass tolerance, max one relaunch
            ...
            ok, tail = poll_until_done(cfg, rs, remote_log, launch_epoch,
                                       args.max_hours, args.poll_seconds)
            if ok:
                success = True
                break
            if attempt == 1 and TWO_PASS_SIG in tail and BENIGN_STEP5 not in tail:
            ...
            rs.fail(error=f"Run did not produce fresh output. Log tail:\n{tail[-1500:]}",
                    hint="Site model stalled on the VM. Restart the run, or contact Jens.")
            write_status(rs)
            return 1

        local_file, note = verify_and_fetch(args.run_id)
        rs.finish_phase(PHASE_KEY, ok=True, note=note)
        rs.output_blob_paths = list(set(rs.output_blob_paths) | {note.split('blob: ')[-1]})
        rs.beat(); write_status(rs)
        print("\n" + rs.timing_summary())
        print(f"\nSUCCESS: {local_file}\n{note}")
        return 0
        ...
        rs.fail(error=str(e),
                hint="Technical failure before/around the VM run. Contact Jens.")
        try:
            write_status(rs)
        except Exception:
            pass
        ...
        if args.keep_vm:
            log.warning("VM left RUNNING (--keep-vm). Remember: ~9 kr/h. Deallocate when done.")
        elif vm_started_here or success:
            deallocate_vm(cfg)
