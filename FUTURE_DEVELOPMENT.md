...
### FD.16 — VM-sidigt auto-shutdown-skyddsnät (oberoende av att den lokala orchestratorn lever)

**Idé:** Lägg en avstängning som deallokerar/stänger VM:en **även om allt lokalt dör** (token-död,
tunnelkollaps, stängd laptop, kraschad runner). Två varianter, enklast först: (a) `shutdown -h +240` på
VM:en vid launch (självstängning efter 4 h oavsett); (b) Azure auto-shutdown-policy på VM:en (deallokerar
schemalagt). Orchestratorns utfallsstyrda deallokering (LB.56) täcker normalfallet — detta är skyddsnätet
för onormalfallet.

**Värde:** Idag, vid observationsförlust (LB.55/56), lämnas VM:en medvetet körande (~9 kr/h) så jobbet
hinner klart. Ett värsta-fall (tunneln död resten av dagen, ingen märker) kan då kosta en helgs VM-tid.
Ett VM-sidigt skyddsnät kapar den svansen utan att riskera att döda en frisk körning. Jens lyfte detta
explicit 2026-06-12 ("auto-avstängning efter säg 4 h, innan PIM").

**Beror på:** Inget IT-beroende för variant (a) — det är ett kommando vid launch. Variant (b) kan kräva
rättighet att sätta auto-shutdown-policy på VM:en (Jens är Owner på RG:n → sannolikt OK).

**Estimerad omfattning:** Låg (variant a: en rad i launcher-skriptet + dokumenterat antagande om maxtid).

**Status:** Idé. Lyft 2026-06-12 under site-körningen ("mer om det senare, inte nu").

**Datum identifierad:** 2026-06-12 / Phase Z site-körning.

---

### FD.17 — Datainmatning end-to-end: lokal extraktion → Blob → VM (DW når ej från Azure, LB.58)

**Idé:** Automatisera hela inmatningskedjan **utan** att VM:en rör DW (vilket är omöjligt, LB.58 — mätt
`BLOCKED`). Den behöriga lokala maskinen (kontorsnät/VPN) kör `export_b4b_for_model.py` +
parquet-regenerering + SQL-dataprep, laddar resultatet till **Blob** (`evipricingmodelstprod`), och
VM:en hämtar från Blob (som den når, `OUT_OK`). Då blir extraktionen en **lokal orchestrator-fas** som
matar VM-faserna — sekvenseraren kör lokala faser lokalt och VM-faser på VM, statusfilen visar hela
kedjan oavsett var varje steg fysiskt kör.

**Värde:** Detta är den enda vägen till "så långt det går via Azure" som respekterar DW:ns
IP-vitlistning (säkerhetsbeslut, ej hinder att kringgå) och kräver **noll IT**. Alternativen utreddes
2026-06-12: (1) extraktion lokal = basfall; (2) VPN på VM = avskrivet (användar-VPN på headless server,
fel konstruktion); (3) vänd flödet, lokal maskin pushar; (4) Blob som mellanlager; (5) öppna
DW-brandväggen för VM = IT, sista utväg. 3+4 vinner — uttömmer icke-IT-vägarna.

**Beror på:** Blob-lagret (finns, testat grönt). `export_b4b_for_model.py` (finns, validerat 0,057 %
drift mot facit). Den frysta-snapshot-kedjan måste köras i rätt ordning (regenerera
`transaction_data.parquet` FÖRST, annars filtreras ny data tyst bort — G7/LB.50-klassen).

**Estimerad omfattning:** Medel (en lokal extraktions-fas-runner + Blob-upload + VM-sidig Blob-pull;
ren orkestrering, ingen ny modellkod). Egen session — Jens har märkt den som nästa kärna.

**Status:** Idé, riktning beslutad. Mätt och utrett 2026-06-12.

**Datum identifierad:** 2026-06-12 / Phase Z DW-åtkomstutredning (LB.58).
